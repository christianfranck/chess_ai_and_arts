...
//var winB=window.open("multiwindows2.html", 'Canvas2');

var canvasA = document.getElementById("renderCanvas");
var canvasB = document.getElementById("renderCanvas2");

//var canvasB = winB.document.getElementById("renderCanvasWin2");
...
function colListener2 (main, collided)
{
    var m=main.object;
    var c=collided.object;
    var pos = c.physicsImpostor.getObjectCenter();
...
//            currentMesh.physicsImpostor.setLinearVelocity(new BABYLON.Vector3(10,30,30));
    ...
    canvasA.addEventListener("pointerdown", onPointerDown, false);
    canvasA.addEventListener("pointerup", onPointerUp, false);
    canvasA.addEventListener("pointermove", onPointerMove, false);

    scene.onDispose = function ()
    {
        canvasA.removeEventListener("pointerdown", onPointerDown);
        canvasA.removeEventListener("pointerup", onPointerUp);
        canvasA.removeEventListener("pointermove", onPointerMove);
    }
...
//        console.log("x: " + x + " y: " + y)
    ...
    //const canvas = document.querySelector('canvasA')
    canvasA.addEventListener('touchstart', function(e) {
        var p =getTouchPosition(canvasA, e);
        e.preventDefault();
        mousedownA=p
 //       console.log("mousedown A "+p);
    })
    canvasA.addEventListener('touchend', function(e) {
        var p =getTouchPosition(canvasA, e);
        e.preventDefault();
        mouseupA=p
        mouseupA=[0,0];
 ...
 //       console.log("mousemove A "+p);
    ...
    canvasB.addEventListener('touchstart', function(e) {
        ...
        e.preventDefault();
        mousedownB=p
        //       console.log("mousedown B "+p);
    })
    canvasB.addEventListener('touchend', function(e) {
        var p =getTouchPosition(canvasB, e);
        e.preventDefault();
        mouseupB=p
        mouseupB=[0,0];
...
var currentMeshA;
var startingPointA;
var startingPositionA;
var humanMoveInProgressA=false;
var currentMeshB;
var startingPointB;
var startingPositionB;
var humanMoveInProgressB=false;

initFunction().then(() =>
{
    var loopcount = 0;
    sceneToRender = scene;
    //engine.inputElement = document.getElementById("renderCanvas");
    engine.runRenderLoop(function ()
    {
        if (sceneToRender && sceneToRender.activeCamera)
        {
            ...
            if (scene.activeCamera.name === "Camera")
            {
                if (mousedownA != null)
                {
                    const pickResult = scene.pick(mousedownA[0], mousedownA[1], function (mesh)
                        {return mesh.isPickable && mesh.name.length==1;}, false, scene.activeCamera);
                    if (pickResult.pickedMesh)
                    {
                        console.log("picked", pickResult.pickedMesh.name);
                        currentMeshA=pickResult.pickedMesh;
                        var pickGround = scene.pick(mousedownA[0], mousedownA[1], function (mesh)
                            { return mesh == ground;});
                        startingPointA=null;
            ...
            if (scene.activeCamera.name === "Camera")
            {
                ...
                if (mousemoveA != null && startingPointA)
                {
                    console.log("dragging");
                    var pickGround = scene.pick(mousemoveA[0], mousemoveA[1], function (mesh)
                    { return mesh == ground;});
                    var current=null;
                    if (pickGround.hit) {current=pickGround.pickedPoint;}
                    if(current)
                    {
                        console.log("moved");
                        var diff = current.subtract(startingPointA);
                        currentMeshA.position.addInPlace(diff);
            ...
            if (scene.activeCamera.name === "Camera")
            {
                if (mouseupA != null && startingPointA)
                {
                    mouseupA = null;
                    ...
                    // if (pickResult.pickedMesh)
                    {
                        ...
                        camera.attachControl(canvasA, true);
                        humanMoveInProgressA=false;

                        startingPointA = null;
                        //  dumpPositions();
                        pieces.forEach(function(p) {
                            if (p!=currentMeshA)
                                currentMeshA.physicsImpostor.unregisterOnPhysicsCollide(p.physicsImpostor, colListener);
                        });
            ...
            if (scene.activeCamera.name === "Camera2")
            {
                if (mousedownB != null)
                {
                    const pickResult = scene.pick(mousedownB[0], mousedownB[1], function (mesh)
                    {return mesh.isPickable && mesh.name.length==1;}, false, scene.activeCamera);
                    if (pickResult.pickedMesh)
                    {
                        console.log("picked", pickResult.pickedMesh.name);
                        currentMeshB=pickResult.pickedMesh;
                        var pickGround = scene.pick(mousedownB[0], mousedownB[1], function (mesh)
                        { return mesh == ground;});
                        startingPointB=null;
                        if (pickGround.hit) {startingPointB=pickGround.pickedPoint;}
                        var x=Math.floor(((currentMeshB.position.x+5)/10)+12);
                        var z=Math.floor(((currentMeshB.position.z+5)/10)+12);
                        startingPositionB= x+z*L;
                        humanMoveInProgressB=true;
                        pieces.forEach(function(p) {
                            if (p!=currentMeshB)
                                currentMeshB.physicsImpostor.registerOnPhysicsCollide(p.physicsImpostor, colListener2);
                        });
            ...
            if (scene.activeCamera.name === "Camera2")
            {
                ...
                if (mousemoveB != null && startingPointB)
                {
                    console.log("dragging");
                    var pickGround = scene.pick(mousemoveB[0], mousemoveB[1], function (mesh)
                    { return mesh == ground;});
                    var current=null;
                    if (pickGround.hit) {current=pickGround.pickedPoint;}
                    if(current)
                    {
                        console.log("moved");
                        var diff = current.subtract(startingPointB);
                        currentMeshB.position.addInPlace(diff);
            ...
            if (scene.activeCamera.name === "Camera2")
            {
                if (mouseupB != null && startingPointB)
                {
                    mouseupB = null;
                    // const pickResult = scene.pick(mouseupA[0], mouseupA[1], undefined, false, scene.activeCamera);
                    // if (pickResult.pickedMesh)
                    {
                        ...
                        //camera.attachControl(canvasA, true);
                        humanMoveInProgressB=false;

                        startingPointB = null;
                        //  dumpPositions();
                        pieces.forEach(function(p) {
                            if (p!=currentMeshB)
                                currentMeshB.physicsImpostor.unregisterOnPhysicsCollide(p.physicsImpostor, colListener2);
                        });

                        whiteToMove= ("PKQBNR".includes(currentMeshB.name));
                        if(whiteToMove) movCntLimitBlack=60*30;
                        else movCntLimitWhite=60*30;

                        var x=Math.floor(((currentMeshB.position.x+5)/10)+12);
                        var z=Math.floor(((currentMeshB.position.z+5)/10)+12);
                        if (startingPositionB=== x+z*L)    whiteToMove= !whiteToMove;
                    }
